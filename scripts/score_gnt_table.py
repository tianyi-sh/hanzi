# -*- coding: utf-8 -*-
"""
对外部数据目录中前 50 个 GNT 文件中的汉字打分，每个 GNT 单独一个评分标签文件。
...
def main(source_dir=None):
    ...
    for i in range(1, NUM_LABELED + 1):
        gnt_name = "%d.gnt" % i
        gnt_path = os.path.join(source_dir, gnt_name)
        online_path = os.path.join(source_dir, "%d_online.csv" % i)
        ...
        label_path = os.path.join(source_dir, "%d_label.txt" % i)
        with open(label_path, "w", encoding="utf-8") as f:
            f.write("%s,%.2f\n" % (char, score))
    print("已保存 1_label.txt ~ %d_label.txt 到目录: %s" % (NUM_LABELED, source_dir))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source-dir", help="包含 GNT 和在线 CSV 的数据目录")

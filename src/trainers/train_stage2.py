...
def train_stage2(cfg_path, run_dir):
    ...
    for ep in range(epochs):
        ...
        align_mod.train()
        decoder.train()
        total_loss = total_mae = total_kl = total_cons = 0.0
        n_b = 0
        for batch in loader:
            traj = batch["traj"]
            B, T, _ = traj.shape
            ...
            if max_k > K:
                pi_prior = torch.nn.functional.pad(pi_prior, (0, max_k - K)).to(traj.device)
            else:
                pi_prior = pi_prior.to(traj.device)
                if max_k < K:
                    z_struct = torch.nn.functional.pad(z_struct, (0, 0, 0, K - max_k))
                    max_k = K
            pi_prior = pi_prior[:, :max_k] / (pi_prior[:, :max_k].sum(1, keepdim=True) + 1e-8)
            ...
            loss = lambda_mae * l_mae + lambda_align * l_align + lambda_cons * l_cons
            opt.zero_grad()
            loss.backward()
            opt.step()
        ...
        log_metrics(log_path, {"epoch": ep, "loss": total_loss / n_b, "recon_mae": avg_mae, "align_kl": total_kl / n_b, "cons_loss": total_cons / n_b}, step=ep)
        if avg_mae < best_mae:
            best_mae = avg_mae
            torch.save({
    ...
    save_metrics_json(os.path.join(run_dir, "metrics.json"), {"recon_mae": best_mae, "cons_loss": total_cons / max(n_b, 1)})
    return best_mae
...
if __name__ == "__main__":
    ...
    train_stage2(os.path.join(ROOT, args.config), run_dir)

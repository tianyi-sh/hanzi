...
def main(source_dir=None):
    ...
    for i in range(1, NUM_ORIGINAL + 1):
        ...
        online_path = os.path.join(source_dir, online_name)
        ...
        for k in range(COPIES_PER_FILE):
            ...
            new_id = next_id + k
            new_gnt_path = os.path.join(source_dir, f"{new_id}.gnt")
            with open(new_gnt_path, "wb") as f:
                f.write(new_gnt_data)
            new_online = add_noise_to_online(online_rows, rng)
            new_online_path = os.path.join(source_dir, f"{new_id}_online.csv")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source-dir", help="要原地扩充的数据目录")
